...
async function verificarPulseira() {
  const selecionada =
    Number(document.getElementById("slct-pulseiras").value) - 1;
  ...
  if (pulseira.situacao === "Alocada") {
    ...
    document.getElementById("span-genero").textContent =
      pulseira.pacienteGenero;
    let dtNasc = new Date(pulseira.pacienteDtNasc);
    document.getElementById("span-nascimento").textContent =
      dtNasc.toLocaleDateString("pt-BR");
...
async function desvincularPaciente() {
  let idx = sessionStorage.getItem('paciente-selecionado');
  let idHospital = JSON.parse(sessionStorage.getItem("enfermeiro")).idHospital;
  if (idx === null || idHospital === null) return false;
  const idPulseiraReal = pulseiras[Number(idx)].id;
  ...
  const resposta = await fetch("/pacientes/remover", {
    ...
    body: JSON.stringify({ id: idPulseiraReal, hospital: idHospital }),
  });

  const resultado = await resposta.text();

  if (resultado === "true") {
    pulseiras[Number(idx)].situacao = "Livre";
    pulseiras[Number(idx)].pacienteNome = null;
    document.getElementById("erro").classList.add("sumir");
    document.getElementById("info").classList.remove("sumir");
